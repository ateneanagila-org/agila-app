...
### Follow-up 4 — Summaries list Original cats only
`commit a78d3e5` — `lib/services/helper.service.ts`. `generateForRiSheet`/`generateForFaSheet` queried cats by region without an `entry_status` filter; once the Original-gate kept unreviewed cats off the region tabs, those cats showed in the summaries as blank-catalog-ID rows. Fix: both summary queries now filter `eq(c.entry_status, "Original")` (matching the census/TNVR stats). For RI additionally excludes terminal-status cats via `or(isNull(c.cat_status), notInArray(c.cat_status, ["Adopted","Deceased","MIA"]))` — mirroring the region-tab mapper, which renders those as "Not Applicable". For FA keeps its `is_adoptable` + `cat_status IS NULL` conditions.

### Verified non-issues
- **`removeSessionCat` orphan** — not a real edge: its only caller ([sessions-create-screen.tsx](../../../components/app-pages/sessions/sessions-create-screen.tsx)) always calls it immediately before `removeCat` (which deletes the cat), and only on `Unsubmitted` session entries that aren't on the sheet. Never used in isolation.

### Follow-up 5 — Column-A suffix self-heal (HOME drift) — 2026-06-06
Sheet-side status edits (a human typing `Deceased` in col L) imported to the DB but never re-stamped col A's suffix, because reverse import cancels forward tasks and never enqueues, and forward sync only re-stamped *tasked* rows. Result: permanent col-A/col-L drift that inflates/deflates the HOME tab (guide §10). Three changes, all on this branch:

1. **Forward sync re-stamps every survivor** ([helper.service.ts](../../../lib/services/helper.service.ts), step 3b). The all-survivor loop that rebuilds col B from DB `photo_url` now *also* recomputes `col A = sheetNumber + statusSuffix(DB cat_status)` for every surviving row (number preserved from the sheet; suffix from the DB). Zero extra query — `findCatsByIds` already returns full `SelectCat`. Guarded `existingNum !== null` (don't stamp unnumbered rows) and inside the `!== "UNKNOWN"` block (UNKNOWN has no suffix layout). So any region with *any* activity fully self-heals its suffixes.
2. **Reverse import enqueues on status change** ([reverse-sync.service.ts](../../../lib/services/reverse-sync.service.ts)). `importSheetRowToDB(data, prevStatus)` now queues a forward re-stamp (via `refreshCatInSyncQueue`, after the GSheet-wins cancel so it's the sole surviving task) when `data.cat_status !== prevStatus`. This wakes an otherwise-idle region so #1 runs next tick. Not a GSheet-wins violation — forward sync keeps the sheet's catalog number and only rewrites the suffix from the just-imported status. Payload col A is `""`; the number always comes from the live sheet row.
3. **Reset script global snap** ([reset-and-reimport.ts](../../../scripts/reset-and-reimport.ts)). After photo import, the script now enqueues every `Original` cat and forward-syncs *every* region (catches truly-dormant regions a plain reverse import would leave stale), then regenerates For RI / For FA from the snapped snapshot. Runs after photo import so col B rebuilds from a populated `photo_url`; freeze-guarded (skips snap + summary regen rather than blanking summaries from an empty snapshot).

Tests: `__tests__/services/forward-sync.test.ts` (+3: re-stamp from DB status, unnumbered-row guard, UNKNOWN skip), new `__tests__/services/reverse-sync.test.ts` (enqueue on change / not on unchanged / not when prevStatus omitted). Together #1+#2 auto-heal any region that ever sees activity; #3 is the belt-and-suspenders for dormant regions at reset.
